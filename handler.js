...
- Loading a downloadable extension prototype.
- Sending legacy catalog rows to a Boltic serverless function.
- Validating messy rows before migration.
- Generating Fynd-style product payloads for review.
- Showing a dry-run ingestion summary without creating products.
...
Dry Run safely simulates migration:

1. Reads the JSON rows from the popup.
2. Maps legacy field names like Product Name, SKU, MRP, and Image URL.
3. Normalizes SKUs, prices, currency, and size variants.
4. Rejects rows with missing names, missing SKUs, duplicate SKUs, invalid prices, missing sizes, or invalid image URLs.
5. Builds Fynd-style payloads for valid products.
6. Returns a simulated success result for each valid product.

Dry Run does not call Fynd, require OAuth, use API keys, create products, or modify catalog data.
